...
export type UseIphonePasswordProps<T extends HTMLTextInputElement> = Props<T>;
...
export type UseIphonePasswordReturn<T extends HTMLTextInputElement> = [
  string,
  string,
  (e: React.ChangeEvent<T>) => NextValueString,
  (genertor: () => string) => void
];

export const useIphonePassword = <T extends HTMLTextInputElement>(
  props?: UseIphonePasswordProps<T>
): UseIphonePasswordReturn<T> => {
  ...
  const generatingValue = (genertor: () => string) => {
    setValue(genertor());
  };
  ...
  return [value, presentation, onChange, generatingValue];
};
